...
`(Scope, Environment, Key)`. The `Scope` and `Environment` values are the scope of an
entry — they control which application and deployment tier owns it.

This page covers `Scope` scoping and the `IncludeScopes` composition. For per-request tenancy as a fourth dimension see [Multi-tenant config](./multi-tenant.md); for the full precedence walk across all four dimensions see [Resolution order](./resolution-order.md).
...
The polling provider fetches all entries where `Scope = "PaymentService"` and
...
scopes. Set `Scope` and `IncludeScopes` with consistent casing across all your hosts.
:::
...
`IncludeScopes` is ordered **lowest-precedence-first**. The `Scope` is always read
...
// Effective: OrgGlobals < PlatformDefaults < Shared < Scope
```
...
WHERE Scope IN ('PlatformDefaults', 'Shared', 'PaymentService')
...
When you mount `MapDbConfigHttp`, you can restrict writes to a specific `Scope` using the
`scopeFilter` parameter. Any request whose route `{scope}` does not match the filter
...
## Overlay scope for deployment variants

A common pattern: the same application is deployed twice with a runtime flag that
differentiates them (e.g. `IsBeta=true` vs `IsBeta=false` set via environment variable
at startup). Most configuration is shared between the two deployments; a handful of
keys need different values for the beta deployment.

Use an **overlay scope** that the beta deployment includes on top of the base:

```
Scope=Api          Environment=Production   ← base. ALL settings.
  ├ Stripe:ApiKey            "sk_live_main"
  ├ FeatureX:Enabled         "true"
  └ ... (everything else)

Scope=Api-Beta     Environment=Production   ← overlay. Only the differences.
  ├ FeatureX:Enabled         "false"
  └ FeatureY:Limit           "5000"
```

```csharp
var isBeta = builder.Configuration.GetValue<bool>("IsBeta");
...
builder.AddDbConfig(b =>
{
    ...
    if (isBeta)
    {
        b.Options.Scope = "Api-Beta";
        b.Options.IncludeScopes = ["Api"];   // base falls through where overlay is silent
    }
    else
    {
        b.Options.Scope = "Api";             // no overlay
    }
    b.UseSqlServer(connStr);
});
```

Both deployments share the bulk of the config from `Api`. The beta deployment's
`Api-Beta` rows win on the few keys it overrides. The pattern scales to additional
variants (`Api-Canary`, `Api-Region-EU`) and to additional apps (`OrderService-Beta`,
`Notifications-Beta`).

Recommended naming convention: `{Scope}-{Variant}` with a dash separator — readable in
the UI, URL-safe in HTTP routes.

## Single-environment hosts

DbConfig's `Environment` column is designed for the case where one database serves
multiple deployment tiers (Production, Staging, Development). If you run a **separate
database per environment** instead, the `Environment` column is dead weight — every row
in your Production DB is by definition Production.

In that case, pick a constant value and use it everywhere:

```csharp
builder.AddDbConfig(b =>
{
    b.Options.Scope = "MyApp";
    b.Options.Environment = "Default";   // constant; not tied to ASP.NET Core's Environment
    b.UseSqlServer(connStr);
});
```

All entries you create through the UI or HTTP API will share this value. The polling
provider's `WHERE Environment = 'Default'` is a no-op narrowing because every row
matches. The Environment column shows `Default` in the admin UI — operators learn to
ignore it.

**Why a constant value works:** if you ever consolidate multiple environments into one
database later, you can switch to real Environment values without a schema migration —
just start writing new rows with `Production` / `Staging` / etc. and let the old
`Default` rows decay or be re-tagged.

**Convention:** use `"Default"`, `"Main"`, `"Production"`, or your organization's
preferred shorthand. The value isn't constrained — pick what reads well in the UI.
...
endpoint; instead, callers pass the union of `Scope` values they want to see:
...
# (no scope narrows to a specific app — every app's rows come back; filter client-side
# by Scope if needed)
```

The flat endpoint returns raw rows ordered `(Scope, Environment, TenantId, Key)`. Each
row keeps its source `Scope`, so the UI can render the "shadowed by override" indicator
...
Every row in the entries table shows a colored scope badge with the source `Scope`.
...
`TenantId` is a fourth scoping dimension alongside `Scope`, `Environment`, and `Key`. Tenant composes with `IncludeScopes`: every entry (own-scope or included-scope) can have a tenant-specific override on top of its global default. Global default entries store `TenantId = ""`.
...
The two dimensions compose with one rule: **tenant axis dominates the scope axis.** A tenant-specific entry beats any global entry regardless of which scope it lives in. Within a single tenant's effective values, the same scope precedence applies as for global (Scope beats IncludeScopes, in array order). See [Resolution order](./resolution-order.md) for the full precedence walk and worked examples; [Multi-tenant config](./multi-tenant.md) covers the resolver model, fallback, schema, and HTTP/UI surfaces.
