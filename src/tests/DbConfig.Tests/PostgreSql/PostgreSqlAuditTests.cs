...
/// <summary>
/// Integration tests for the audit log on PostgreSQL.
...
public sealed class PostgreSqlAuditTests : IAsyncLifetime
{
    ...
    public PostgreSqlAuditTests(PostgreSqlFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task SecretEntry_AuditRowStoresCiphertext()
    {
        ...
        var raw = await context.AuditEntries
            .AsNoTracking()
            .Where(x => x.Scope == App && x.Environment == Env && x.Key == "SecretAuditKey")
